...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   TBranch        *b_hit_process_key;   //!//! \param tree

    Int_t *fEvent_ID;
    std::vector<Long64_t>*fVoxelID;
    std::vector<double>*fVoxel_x;
    std::vector<double>*fVoxel_y;
    std::vector<double>*fVoxel_z;
    std::vector<double>*fVoxel_Q;
   ...
   virtual void     Show(Long64_t entry = -1);
    std::map<Long64_t,Voxel*> VoxelizetheHits(Int_t,std::string);
   void PlotHits(Int_t EventID,Int_t Pdg_Code);
   void Getdedx(Int_t Pdg_Code,Int_t limit,Int_t PionHitLimit);
   std::vector<HitMap*> GetAllHits(Int_t,Long64_t);
    HitMap* GetSingleEventHits(Long64_t jentry);
    void VoxelFloor(double XTrue,double YTrue,double ZTrue,double XStep,double YStep,double ZStep,double StartingSize,std::map<Long64_t,Voxel*> &Voxels );
    void Clear();
